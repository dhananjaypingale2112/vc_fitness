	...
	public function index() {
		...
		if ($product_info) {
         ...
         $data['description'] = $this->shortcodes->do_shortcode($data['description']);
			...
			$data['pincode'] = isset($this->session->data['pincode']) ? $this->session->data['pincode'] : "";
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						...
						if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$option_value['special_price']) {
							$sprice = $this->currency->format($this->tax->calculate($option_value['special_price'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false));
						} else {
							$sprice = false;
						}
			...
			//weight option
			$data['weight_options'] = array();

			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id'], 'weight') as $option) {
				$product_option_value_data = array();

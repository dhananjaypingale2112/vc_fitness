		...
		if ($category_info) {
         ...
         $data['description'] = $this->shortcodes->do_shortcode($data['description']);
			...
			foreach ($results as $result) {
				...
				$second_img = $this->model_catalog_product->getProductImages($result['product_id']);
				
				if ($result['image']) {
					$image  = $this->model_tool_image->resize($result['image'], $this->config->get('config_image_product_width'), $this->config->get('config_image_product_height'));
					if($second_img){
						$image2 = $this->model_tool_image->resize($second_img[0]['image'], $this->config->get('config_image_product_width'), $this->config->get('config_image_product_height'));
					}else{
						$image2='';
					}
				} else {

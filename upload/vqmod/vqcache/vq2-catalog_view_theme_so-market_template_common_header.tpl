...
if(!isset($_SESSION['pincodeStatus'])){
	$_SESSION['pincodeStatus'] = 0;
}else{
	$_SESSION['pincodeStatus'] = 1;
}
...
<?php // Begin Themes Scripts ----?>
	...
	<?php } ?>
	<link rel="stylesheet" type="text/css" href="catalog/view/theme/so-market/css/bootstrap-datepicker.css">
	<script type="text/javascript" src="catalog/view/theme/so-market/js/bootstrap-datepicker.js"></script>
...
<div id="wrapper" class="<?php echo $cls_wrapper; ?>">
				...
				<div class="row">
					<div class="header-top-left form-inline col-sm-6 compact-hidden">
						...
						if($soconfig_general['phone_status']){
							if (isset($soconfig_general[$lang]["contact_number"]) && is_string($soconfig_general[$lang]["contact_number"])) {
							 echo '<div class="form-group navbar-phone" style="padding:0 10px"><i class="fa fa-phone"></i> '.html_entity_decode($soconfig_general[$lang]["contact_number"], ENT_QUOTES, 'UTF-8').'</div>';
							} 
						}
						?>
						...
						<?php endif; ?>
						<div class="form-group navbar-pincode hidden-xs" style="padding:0 10px;">
							<button type="button" onclick="check_pincode_fun();" class="btn btn-default" style="text-align: center;line-height: 0px;"><i class="fa fa-location-arrow" aria-hidden="true"></i><span id="pin"> <?php if(isset($pincode) && !empty($pincode)) { echo $pincode; } else { echo "Change"; } ?></span></button>
						</div>
					...
					<div class="header-top-right collapsed-block col-xs-6 compact-hidden">
						...
						<ul class="top-link list-inline">
							<li class="account" id="my_account"><a href="<?php echo $account; ?>" title="<?php echo $text_account; ?>" class="btn btn-xs dropdown-toggle" data-toggle="dropdown"> <span class="hidden-xs"><?php if(isset($session->data['customer_custom_details']['firstname']) && $session->data['customer_custom_details']['firstname']!=""){ echo "Hi,".ucfirst($session->data['customer_custom_details']['firstname']); } else { echo $text_account; } ?></span> <span class="fa fa-angle-down"></span></a>
							...
							<?php if($soconfig_general["wishlist_status"]):?><li class="wishlist"><a href="<?php echo $wishlist; ?>" id="wishlist-total" class="top-link-wishlist" title="<?php echo $text_wishlist; ?>"><span class="hidden-xs"><?php echo $text_wishlist; ?></span></a></li><?php endif; ?>
			...
			<div class="container">
			   ...
			   <?php if ($header_type == 'center') { ?>
					...
					<?php if ($soconfig_general['searchbox'])
						  echo $content_search; ?>
					<div class="navbar-logo col-md-5 col-sm-12 col-xs-12 text-center">
						<?php if ($logo) { ?>
	...
	var session = <?php echo json_encode($_SESSION['pincodeStatus']); ?>;

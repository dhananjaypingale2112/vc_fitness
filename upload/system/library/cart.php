		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				foreach (json_decode($cart['option']) as $product_option_id => $value) {
					...
					if ($option_query->num_rows) {
						if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image') {
							$option_value_query = $this->db->query("SELECT pov.option_value_id, ovd.name, pov.quantity, pov.subtract, pov.price, pov.price_prefix, pov.special_price, pov.special_price_prefix, pov.points, pov.points_prefix, pov.weight, pov.weight_prefix FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_option_value_id = '" . (int)$value . "' AND pov.product_option_id = '" . (int)$product_option_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
							...
							if ($option_value_query->num_rows) {
								...
								if ($option_value_query->row['special_price_prefix'] == '+') {
									$option_sprice += $option_value_query->row['special_price'];
								} elseif ($option_value_query->row['special_price_prefix'] == '-') {
									$option_sprice -= $option_value_query->row['special_price'];
								}
								...
								$option_data[] = array(
									...
									'special_price_prefix'    => $option_value_query->row['special_price_prefix'],
				...
				$product_data[] = array(
					...
					'price'			  => ($option_price ? $option_price : ($option_sprice ? $option_sprice : $price)),
					'total'			  => $cart['quantity'] * ($option_price ? $option_price : ($option_sprice ? $option_sprice : $price)),

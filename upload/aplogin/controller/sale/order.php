...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_batch_date'])) {
			$filter_batch_date = $this->request->get['filter_batch_date'];
		} else {
			$filter_batch_date = null;
		}
		...
		if (isset($this->request->get['filter_batch_date'])) {
			$url .= '&filter_batch_date=' . urlencode(html_entity_decode($this->request->get['filter_batch_date'], ENT_QUOTES, 'UTF-8'));
		}
		...
		$batch_values = $this->model_sale_batch->getbatchName($batch_id);
		$batch_name = $batch_values[0]['batch_name'];
			$data['orders'][] = array(
		...
		$data['shipping_batches'] = $this->model_sale_batch->getBatch();
		$data['sort'] = $sort;
		$data['order'] = $order;
	...
	public function invoice() {
		...
		$data['column_price'] = $this->language->get('column_price');
		$data['column_total'] = $this->language->get('column_total');
		$data['text_shipping_time'] = $this->language->get('text_shipping_time');
		...
		foreach ($orders as $order_id) {
			...
			$batch_name = $batch_values[0]['batch_name'];
			$data['batch_name'] = $batch_name;
			$data['batch_date'] = $order_info['batch_date'];
			if ($order_info) {
	...
	public function shipping() {
		...
		foreach ($orders as $order_id) {
			...
			$batch_values = $this->model_sale_batch->getbatchName($batch_id);
			$batch_name = $batch_values[0]['batch_name'];
			$batch_date = $order_info['batch_date'];
			$data['batch_name'] = $batch_name;

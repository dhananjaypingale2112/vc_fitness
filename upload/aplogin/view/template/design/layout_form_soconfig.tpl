        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-layout" class="form-horizontal">
          ...
          <table id="route" class="table table-striped table-bordered table-hover">
            ...
            <tbody>
              ...
              if($layout_route['route'] == 'common/home') {
              $is_home = TRUE;
              } ?>
          ...
          <table id="module" class="table table-striped table-bordered table-hover">
            ...
            <tbody>
              ...
              <tr id="module-row<?php echo $module_row; ?>">
                ...
                <?php if ($name == 'Footer' || $name == 'footer') { ?> 
                 ...
                 <?php } else { ?>   
                    ...
                    <?php } ?>
					<?php if ($layout_module['position'] == 'content_search') { ?>
                    <option value="content_search" selected="selected">Content Search</option>
                    <?php } else { ?>
                    <option value="content_search">Content Search</option>
                    <?php } ?>
					<?php if ($layout_module['position'] == 'content_menu') { ?>
                    <option value="content_menu" selected="selected">Content Menu</option>
                    <?php } else { ?>
                    <option value="content_menu">Content Search</option>
                    <?php } ?>
                    <?php if ($layout_module['position'] == 'content_block1') { ?>
                    <option value="content_block1" selected="selected">Content Block 1</option>
                    <?php } else { ?>
                    <option value="content_block1">Content Block 1</option>
                    <?php } ?>
					
					<?php if (isset($is_home)) { ?>
                    <?php if ($layout_module['position'] == 'content_block2') { ?>
                    <option value="content_block2" selected="selected">Content Block 2</option>
                    <?php } else { ?>
                    <option value="content_block2">Content Block 2</option>
                    <?php } ?>
                    <?php if ($layout_module['position'] == 'content_block3') { ?>
                    <option value="content_block3" selected="selected">Content Block 3</option>
                    <?php } else { ?>
                    <option value="content_block3">Content Block 3</option>
                    <?php } ?>
                    <?php if ($layout_module['position'] == 'content_block4') { ?>
                    <option value="content_block4" selected="selected">Content Block 4</option>
                    <?php } else { ?>
                    <option value="content_block4">Content Block 4</option>
                    <?php } ?>
                    <?php } ?>
...
function addModule() {
	...
	html += '    <option value="content_search">Content Search</option>';
	html += '    <option value="content_menu">Content Menu</option>';
	html += '    <option value="content_block1">Content Block 1</option>';
	<?php if (isset($is_home)) { ?>
	html += '    <option value="content_block2">Content Block 2</option>';
	html += '    <option value="content_block3">Content Block 3</option>';
	html += '    <option value="content_block4">Content Block 4</option>';
	<?php } ?>
    html += '  </select></td>';
